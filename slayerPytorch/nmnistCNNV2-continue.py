...
# Funkcia pre nacitanie natrenovaneho modelu
def load_checkpoint(model, optimizer, filename):
    start_epoch = 0
    if os.path.isfile(filename):
        print("=> nacitavanie modelu '{}'".format(filename))
        checkpoint = torch.load(filename)     # Nacitanie modelu
        start_epoch = checkpoint['epoch'] + 1 # Nacitanie epoch
        model.load_state_dict(checkpoint['state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer'])
        stats = checkpoint['stats']  # Nacitanie statistickych udajov
        print("=> model bol nacitany '{}' (epoch {})"
                  .format(filename, checkpoint['epoch']))
    else:
        print("=> model nenajdeny '{}'".format(filename))
...
model_path = "CESTA K SUBORU .pth"
net , optimizer, stats, start_epoch = load_checkpoint(net, optimizer, model_path)
